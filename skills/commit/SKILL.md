...
Establish commit authority from an explicit user request or authorised
...
Partition eligible changes by coherent intent using **atomic commits**.
...
For each concern, retain an already exact staged selection or use
...
Write an accurate Conventional Commit message in the form
`type[(scope)][!]: description` for the staged diff.

### 5. Create the commit

Create the commit and allow configured Git hooks to run. On any failure,
...
Verify each created commit against its recorded diff, inspect the remaining
