...
When a material external subject could change the disposition and needs durable evidence, invoke `$research` with that bounded subject, then read its report before using its findings. Invoke `$maintain-ubiquitous-language` only when the evidence independently confirms consequential durable project language and `$maintain-decision-records` only when it confirms qualifying rationale; a proposed classification or route alone does not establish either. The evidence for classification, duplicate confidence, and remaining uncertainty is explicit.
