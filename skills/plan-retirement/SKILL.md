...
Invoke `$research` only when a material external legal, regulatory, provider, protocol, standards, or deprecation claim could change the plan and its evidence must survive the session. Read and verify its cited report before relying on the conclusion; treat research as evidence, not authority to invent a project obligation. Invoke `$maintain-context` inline when independently confirmed consequential project language is introduced, changed, misused, or contradicted; defer any design-dependent terminology or qualifying ADR until the complete retirement synthesis is confirmed. Each utility owns only its contract-specific result and returns control here.
