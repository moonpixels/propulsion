...
description: Maintains a project's ubiquitous language and architecture decision records. Use when domain terms or consequential codebase decisions emerge or change.
metadata:
    invocation: model
...
**Ubiquitous Language**, from Eric Evans's domain-driven design, keeps project
language rigorous by using, testing, and refining the domain model in everyday
work; concise architecture decision records preserve the consequential choices
...
1. Compare domain language in the conversation with the root `CONTEXT.md` and
   relevant code.
   Challenge vague, overloaded, or conflicting terms immediately, propose one
   precise canonical term, and test it with concrete scenarios and edge cases.
   Treat code as evidence of current behaviour and the user's confirmed answer
   as intent; surface contradictions for resolution. The term is either
