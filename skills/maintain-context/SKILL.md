...
Compare domain language in the conversation with the root `CONTEXT.md` and
...
As soon as a term is validated and agreed, create or update the single root
...
After a qualifying decision is accepted, create `docs/adr/` lazily and write
...
When an accepted decision changes, read only the specific ADR it supersedes,
...
Re-read every changed artifact against the resolved language, accepted
