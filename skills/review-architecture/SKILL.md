...
Establish the review scope. Use the user's explicit scope or the whole
...
Recover quality drivers from explicit project evidence, then apply the
...
Use the reference's diagnostics to map cohesive capabilities, public
...
Apply **design it twice** to every serious candidate. Compare at least two
...
Invoke `$research` when a candidate materially depends on an external
...
Evaluate candidates with explained `high`, `medium`, or `low` impact,
...
For each recommendation, define the evidenced problem, affected modules and
...
Read [Report Design](references/report-design.md), then create one single-file
