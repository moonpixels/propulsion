...
Invoke `$elicit` only when a material scope, priority, or readiness decision depends on user-held knowledge or authority. Supply inspected evidence, viable options, and consequences; let it ask one question at a time and use only its confirmed synthesis. Retain responsibility for the complete proposal, mutation, verification, and stopping.
