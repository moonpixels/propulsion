...
Locate the repository root and inspect the root `AGENTS.md`, applicable
...
Classify every existing and proposed instruction. Keep only succinct
...
Ensure the file states: `When the user corrects you with a reusable,
...
Create or rewrite only the root `AGENTS.md`. Use direct imperative lines,
...
Re-read every line against its repository-wide scope and runtime value.
